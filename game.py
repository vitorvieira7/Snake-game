...
def main_game_loop():
    ...
    enemies = [
        [
            random.randrange(1, WIDTH // enemy_size) * enemy_size,
            random.randrange(1, HEIGHT // enemy_size) * enemy_size,
        ]
    ]
    ...
    def draw_snake(snake_body):
        for part in snake_body:
            pygame.draw.rect(
                screen, GREEN, pygame.Rect(part[0], part[1], snake_size, snake_size)
            )

    def draw_enemies(enemies):
        for enemy in enemies:
            pygame.draw.rect(
                screen, RED, pygame.Rect(enemy[0], enemy[1], enemy_size, enemy_size)
            )
    ...
    while not game_over:
        ...
        pygame.draw.rect(
            screen, WHITE, pygame.Rect(food_pos[0], food_pos[1], snake_size, snake_size)
        )
    ...
    show_message("Aperte R para reiniciar", WHITE, 50)
    show_message("Aperte Q para sair", WHITE, 100)
    pygame.display.update()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_r:
                    main_game_loop()  # Reinicia chamando a função novamente
